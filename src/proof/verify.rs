...
mod tests {
    ...
    fn empty_trie() {
        ...
        assert_eq!(verify_proof(proof.into_values(), root, key, vec![]), Ok(()));
    ...
    fn single_leaf_trie_proof_verifcation() {
        ...
        let proof = hash_builder.take_proofs();
        assert_eq!(verify_proof(proof.into_values(), root, target, target.to_vec()), Ok(()));
    }
    ...
    fn wide_trie_proof_verification() {
        ...
        let proof1 = proof
            .clone()
            .into_iter()
            .filter_map(|(k, v)| Nibbles::unpack(target1).starts_with(&k).then_some(v));
        assert_eq!(verify_proof(proof1, root, target1, target1.to_vec()), Ok(()));

        let proof2 = proof
            .clone()
            .into_iter()
            .filter_map(|(k, v)| Nibbles::unpack(target2).starts_with(&k).then_some(v));
    ...
    fn arbitrary_proof_verification() {
        ...
        proptest!(|(state: std::collections::BTreeMap<B256, alloy_primitives::U256>)| {
            ...
            for (key, value) in hashed {
                let proof = proofs.iter().filter_map(|(k, v)| Nibbles::unpack(key).starts_with(k).then_some(v.clone()));
